...
const fillSheetWithOrders = (
  ...
  let priceLookupRow: Record<string, number> = {};

  funds.forEach((fund, index) => {
    priceLookupRow[fund] = index + 10;
  });
  ...
  funds.forEach((fund) => {
    ...
    fundOrders.forEach((order) => {
      const values = [
        ...
        order.quantity,
        {
          formula: `$N$${priceLookupRow[order.fund]}`,
          date1904: false,
        },
        order.average_price,
        order.amount,
  ...
  funds.forEach((fund) => {
    let latestOrder = maxBy(ordersByFund[fund], "date")!;
    worksheet.getRow(priceLookupRow[fund]).getCell('M').value = fund;
    worksheet.getRow(priceLookupRow[fund]).getCell('N').value = latestOrder.average_price;
  });
